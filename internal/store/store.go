...
func (s *Store) Transfer(ctx context.Context, resource, currentHolder string, fencingToken, ttlSec, now int64, newHolder string) (LeaseRow, error) {
	...
	expiresAt := now + ttlSec
	if _, err := tx.ExecContext(ctx,
		`UPDATE leases SET holder = ?, fencing_token = ?, expires_at = ?, ttl_seconds = ? WHERE resource = ?`,
		newHolder, token, expiresAt, ttlSec, resource); err != nil {
...
func (s *Store) ListByHolder(ctx context.Context, holder string) ([]LeaseRow, error) {
	rows, err := s.db.QueryContext(ctx, selectLeaseSQL+` WHERE holder = ? ORDER BY expires_at ASC`, holder)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
